...
func TestParseTenantDefaults(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			input: "",
			want:  models.TenantDefaults{DuplicateGuard: true, CleanupScanEnabled: true},
		},
		{
			name:  "both toggles off",
			input: "duplicate_guard=false,cleanup_scan_enabled=false",
			want:  models.TenantDefaults{DuplicateGuard: false, CleanupScanEnabled: false},
		},
		{
			name:  "partial: duplicate_guard off, rest keep bundle",
			input: "duplicate_guard=false",
			want:  models.TenantDefaults{DuplicateGuard: false, CleanupScanEnabled: true},
		},
		{
		...
		{
			name:    "retired staleness key fails",
			input:   "staleness=hard",
			wantErr: `unknown key "staleness"`,
		},
		{
...
func TestLoadTenantDefaultsBundle(t *testing.T) {
	...
	t.Run("env override wins", func(t *testing.T) {
		t.Setenv("MEMORY_DEFAULT_OPTS", "duplicate_guard=false,cleanup_scan_enabled=false")
		cfg, err := Load()
		if err != nil {
			t.Fatalf("load: %v", err)
		}
...
// TestLoadRejectsNonPositiveEmbeddingDimensions guards the fail-fast contract for
// EMBEDDING_DIMENSIONS: a non-positive value can't size the vector column.
func TestLoadRejectsNonPositiveEmbeddingDimensions(t *testing.T) {
	cases := []struct {
		name    string
		value   string
		wantErr string
	}{
		{name: "default unset ok (768)", value: ""},
		{name: "positive ok", value: "1024"},
		{name: "zero rejected", value: "0", wantErr: "EMBEDDING_DIMENSIONS must be > 0"},
		{name: "negative rejected", value: "-1", wantErr: "EMBEDDING_DIMENSIONS must be > 0"},
	}
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			if tc.value != "" {
				t.Setenv("EMBEDDING_DIMENSIONS", tc.value)
			}
			cfg, err := Load()
			if tc.wantErr != "" {
				if err == nil || !strings.Contains(err.Error(), tc.wantErr) {
			...
			if cfg.EmbeddingDimensions <= 0 {
				t.Fatalf("loaded non-positive EmbeddingDimensions: %d", cfg.EmbeddingDimensions)
			}
		})
	}
}

// TestLoadRejectsCandidatePoolOutOfRange guards the fail-fast contract for
// MEMORY_CANDIDATE_POOL: must be in [1, maxCandidatePool] (bounds the O(n²) MMR).
func TestLoadRejectsCandidatePoolOutOfRange(t *testing.T) {
	cases := []struct {
		name    string
		value   string
		wantErr string
	}{
		{name: "default unset ok (20)", value: ""},
		{name: "min ok", value: "1"},
		{name: "max ok", value: "1000"},
		{name: "zero rejected", value: "0", wantErr: "MEMORY_CANDIDATE_POOL must be in [1, 1000]"},
		{name: "negative rejected", value: "-3", wantErr: "MEMORY_CANDIDATE_POOL must be in [1, 1000]"},
		{name: "absurd upper rejected", value: "100000", wantErr: "MEMORY_CANDIDATE_POOL must be in [1, 1000]"},
	}
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			if tc.value != "" {
				t.Setenv("MEMORY_CANDIDATE_POOL", tc.value)
			}
			cfg, err := Load()
			if tc.wantErr != "" {
				if err == nil || !strings.Contains(err.Error(), tc.wantErr) {
			...
			if cfg.CandidatePool < 1 || cfg.CandidatePool > 1000 {
				t.Fatalf("loaded out-of-range CandidatePool: %d", cfg.CandidatePool)
			}
		})
	}
}
