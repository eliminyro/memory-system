...
// All returns a fresh copy of every doc_type -> day threshold, reusing the same
// cache and refresh as DaysFor. The copy keeps callers from mutating the cache.
func (s *ThresholdStore) All(ctx context.Context) (map[string]int, error) {
	...
	s.mu.RLock()
	defer s.mu.RUnlock()
	out := make(map[string]int, len(s.cache))
