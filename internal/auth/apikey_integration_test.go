...
func openAuthPG(t *testing.T) *gorm.DB {
	...
	if err := database.Migrate(db, "fake", "fake", apiKeyTestDim, database.TenantColumnDefaults{StalenessMode: "off"}, database.BaselineGlobalConfigDefaults()); err != nil {
...
func TestValidateKeyRealDB(t *testing.T) {
	...
	tenant := models.Tenant{ID: uuid.New(), Name: "apikey-" + uuid.NewString()}
	...
	// Valid key: resolves to owning tenant + service-principal subject.
	...
	// API keys no longer carry a tenant email (email lives on tenant_users).
	if info.Email != "" {
		t.Errorf("Email = %q, want empty (no tenant email on the key path)", info.Email)
	}
