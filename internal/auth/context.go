...
// WithLocalAdmin marks a context as an in-process, inherently-privileged local
// admin with no Subject to resolve; the admin gate honors it so callers reuse
// the same tuple-seeding lifecycle methods as the network paths.
//
// SECURITY: only in-process callers set this. Network entry points (MCP, HTTP)
