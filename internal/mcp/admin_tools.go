...
type CreateTenantInput struct {
	...
	OwnerEmail string `json:"owner_email,omitempty" jsonschema:"Owner email for a personal tenant; provisions its owner mapping. Ignored for shared."`
...
func (s *Server) registerAdminTools(srv *mcpsdk.Server) {
	...
	mcpsdk.AddTool(srv, &mcpsdk.Tool{
		Name:        "get_doc_type_policies",
		Description: "List the per-doc_type policy rows and their resolved effective values (NULLs on a row mark inherited values). Instance admin only.",
	}, s.GetDocTypePolicies)

	mcpsdk.AddTool(srv, &mcpsdk.Tool{
		Name:        "set_doc_type_policy",
		Description: "Write one doc_type policy row (curation, embed/search, write_mode, identity rules). NULL fields inherit from reference. Validated against the merged result; applies instance-wide immediately. Instance admin only.",
	}, s.SetDocTypePolicy)
...
func (s *Server) CreateTenant(ctx context.Context, _ *mcpsdk.CallToolRequest, input CreateTenantInput) (*mcpsdk.CallToolResult, any, error) {
	...
	if input.Type == models.TenantTypePersonal && input.OwnerEmail == "" {
		return errorResult("owner_email is required for a personal tenant"), nil, nil
	}
	...
	tenant, err := s.memory.CreateTenant(ctx, input.Name, input.OwnerEmail, input.Type)
	if err != nil {
		return handleAdminError(err)
	}
...
func (s *Server) GetDocTypePolicies(ctx context.Context, _ *mcpsdk.CallToolRequest, _ GetDocTypePoliciesInput) (*mcpsdk.CallToolResult, any, error) {
	rows, eff, err := s.memory.ListDocTypePolicies(ctx)
	if err != nil {
		return handleAdminError(err)
	}
	return jsonResult(map[string]any{"rows": rows, "effective": eff}), nil, nil
}
...
func (s *Server) SetDocTypePolicy(ctx context.Context, _ *mcpsdk.CallToolRequest, input SetDocTypePolicyInput) (*mcpsdk.CallToolResult, any, error) {
	...
	if err := s.memory.SetDocTypePolicy(ctx, row); err != nil {
		return handleAdminError(err)
	}
	return jsonResult(map[string]any{"status": "ok", "doc_type": input.DocType}), nil, nil
}

func writeModePtr(s *string) *models.WriteMode {
	if s == nil {
		return nil
	}
	w := models.WriteMode(*s)
	return &w
}

func slugFormatPtr(s *string) *models.SlugFormat {
	if s == nil {
		return nil
	}
	f := models.SlugFormat(*s)
	return &f
}

func subcategoryPtr(s *string) *models.SubcategoryRule {
	if s == nil {
		return nil
	}
	r := models.SubcategoryRule(*s)
	return &r
}
