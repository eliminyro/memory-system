...
func (s *Server) registerTools(srv *mcpsdk.Server) {
	...
	mcpsdk.AddTool(srv, &mcpsdk.Tool{
		Name:        "link_documents",
		Description: "Create a directed typed edge between two documents. edge_type is one of supersedes, derived_from, relates_to. A supersedes edge auto-archives the target (archive_reason=superseded). Requires editor on the source; supersedes also requires editor on the target, others need only viewer. Idempotent: re-creating an identical edge returns the existing one with no side effect.",
	}, s.LinkDocuments)
...
func (s *Server) UpdateMyTenantSettings(ctx context.Context, _ *mcpsdk.CallToolRequest, input UpdateMyTenantSettingsInput) (*mcpsdk.CallToolResult, any, error) {
	tenant, err := s.memory.UpdateMyTenantSettings(ctx, input.StalenessMode, input.DuplicateGuard, input.DuplicateThreshold, false, input.CleanupScanEnabled)
	if err != nil {
