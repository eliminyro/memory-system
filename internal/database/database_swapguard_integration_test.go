...
func TestMigrateEmbeddingIdentityGuard(t *testing.T) {
	...
	require.NoError(t, database.Migrate(db, "fake", "model-a", swapDim, td))
	...
	err := database.Migrate(db, "fake", "model-b", swapDim, td)
	require.Error(t, err)
