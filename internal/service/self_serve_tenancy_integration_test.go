...
func TestProvisionPersonalTenantGatePass(t *testing.T) {
	...
	id, err := svc.ProvisionPersonalTenant(context.Background(), email, "", "", []string{"example.com"}, iss, sub)
	require.NoError(t, err)
	...
	// Exactly one owner mapping, anchored to (issuer, subject) from the start (D4).
	...
	require.NotNil(t, users[0].Issuer)
	require.NotNil(t, users[0].Subject)
	require.Equal(t, iss, *users[0].Issuer)
	require.Equal(t, sub, *users[0].Subject)
...
// TestCreateTenantPersonalProvisionsOwnerMapping proves an admin-created personal
// tenant carries exactly one owner mapping (email set, subjectless until first
// login), while a shared tenant gets none even if an owner email is supplied.
func TestCreateTenantPersonalProvisionsOwnerMapping(t *testing.T) {
	db := openServicePG(t)
	store := authz.NewPostgresStore(db)
	svc := newAdminTestSvc(db, store)
	adminCtx := auth.WithLocalAdmin(context.Background())
	...
	tn, err := svc.CreateTenant(adminCtx, "personal-own-"+uuid.NewString(), ownerEmail, models.TenantTypePersonal)
	require.NoError(t, err)

	users, err := svc.ListTenantUsers(adminCtx, tn.ID)
	require.NoError(t, err)
	require.Len(t, users, 1)
	require.Equal(t, ownerEmail, users[0].Email)
	require.Equal(t, models.TenantUserRoleOwner, users[0].Role)
	require.Nil(t, users[0].Issuer, "admin-created owner mapping starts subjectless")
	require.Nil(t, users[0].Subject)
	requireTuple(t, store, authzseed.TenantOwner(tn.ID, users[0].ID.String()))

	sh, err := svc.CreateTenant(adminCtx, "shared-noown-"+uuid.NewString(), "ignored@example.com", models.TenantTypeShared)
	require.NoError(t, err)
	shUsers, err := svc.ListTenantUsers(adminCtx, sh.ID)
	require.NoError(t, err)
	require.Empty(t, shUsers, "shared tenants carry no owner mapping")
}
