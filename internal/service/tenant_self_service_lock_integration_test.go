...
func TestSelfServiceToggleEditMatrix(t *testing.T) {
	...
	require.NoError(t, err)
	_, err = svc.UpdateMyTenantSettings(ctxFor(personal.ID, ownerTU.ID.String()), strPtr(models.StalenessModeHard), nil, nil)
	...
	_, err = svc.UpdateMyTenantSettings(sysCtx, strPtr(models.StalenessModeHard), nil, nil)
	...
	require.NoError(t, err)
	_, err = svc.UpdateMyTenantSettings(ctxFor(sharedLocked.ID, adminTU.ID.String()), strPtr(models.StalenessModeAdvisory), nil, nil)
	...
	require.NoError(t, err)
	_, err = svc.UpdateMyTenantSettings(ctxFor(sharedLocked.ID, memTU.ID.String()), strPtr(models.StalenessModeAdvisory), nil, nil)
...
func TestSelfServicePolicyAdminOnlySetter(t *testing.T) {
	...
	require.NoError(t, err)
	_, err = svc.UpdateMyTenantSettings(ctxFor(tenant.ID, ownerTU.ID.String()), strPtr(models.StalenessModeAdvisory), nil, nil)
	require.NoError(t, err)
	tenants, err := svc.ListTenants(adminCtx)
	require.NoError(t, err)
	for _, tn := range tenants {
...
func TestSelfServiceDefaultOpenRegression(t *testing.T) {
	...
	_, err = svc.UpdateMyTenantSettings(ownerCtx, strPtr(models.StalenessModeAdvisory), nil, nil)
