...
func TestSelfServiceToggleEditMatrix(t *testing.T) {
	...
	require.NoError(t, err)
	_, err = svc.UpdateMyTenantSettings(ctxFor(shared.ID, openAdminTU.ID.String()), bptrLocal(true), nil, false, nil, nil)
	...
	require.NoError(t, err)
	_, err = svc.UpdateMyTenantSettings(ctxFor(personal.ID, ownerTU.ID.String()), bptrLocal(true), nil, false, nil, nil)
	...
	_, err = svc.UpdateMyTenantSettings(sysCtx, bptrLocal(true), nil, false, nil, nil)
	...
	require.NoError(t, err)
	_, err = svc.UpdateMyTenantSettings(ctxFor(sharedLocked.ID, adminTU.ID.String()), bptrLocal(true), nil, false, nil, nil)
	...
	require.NoError(t, err)
	_, err = svc.UpdateMyTenantSettings(ctxFor(sharedLocked.ID, memTU.ID.String()), bptrLocal(true), nil, false, nil, nil)
...
func TestSelfServicePolicyAdminOnlySetter(t *testing.T) {
	...
	require.NoError(t, err)
	_, err = svc.UpdateMyTenantSettings(ctxFor(tenant.ID, ownerTU.ID.String()), bptrLocal(true), nil, false, nil, nil)
	require.NoError(t, err)
	tenants, err := svc.ListTenants(adminCtx)
	require.NoError(t, err)
	for _, tn := range tenants {
...
func TestSelfServiceDefaultOpenRegression(t *testing.T) {
	...
	_, err = svc.UpdateMyTenantSettings(ownerCtx, bptrLocal(true), nil, false, nil, nil)
