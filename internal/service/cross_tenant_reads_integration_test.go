...
func TestCrossTenantReads_NoLeak(t *testing.T) {
	...
	results, err := f.svc.Search(ctxA, token, nil, nil, nil, 20, false, "", nil, false)
	require.NoError(t, err)
	...
	docs, err := f.svc.ListDocuments(ctxA, nil, nil, nil, service.ListOptions{})
	require.NoError(t, err)
	for _, d := range docs {
	...
	filtered, err := f.svc.Search(ctxA, token, nil, nil, nil, 20, false, "", &f.tenantB, false)
	require.NoError(t, err)
...
func TestCrossTenantReads_FreshTenantPrivateByConstruction(t *testing.T) {
	...
	results, err := f.svc.Search(ctxS, token, nil, nil, nil, 20, false, "", nil, false)
	require.NoError(t, err)
	for _, r := range results {
		require.NotEqual(t, newT.ID, r.TenantID, "fresh tenant's doc must not leak via search")
	}
	docs, err := f.svc.ListDocuments(ctxS, nil, nil, nil, service.ListOptions{})
	require.NoError(t, err)
	for _, d := range docs {
	...
	filtered, err := f.svc.Search(ctxS, token, nil, nil, nil, 20, false, "", &newT.ID, false)
	require.NoError(t, err)
...
func TestCrossTenantReads_LabeledByOwningTenant(t *testing.T) {
	...
	results, err := f.svc.Search(ctxA, "betaonly", nil, nil, nil, 20, false, "", nil, false)
	require.NoError(t, err)
	var sawB bool
	for _, r := range results {
