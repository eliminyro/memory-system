...
func newVecTestSvc(db *gorm.DB, store authz.Store, emb service.EmbeddingProvider) *service.MemoryService {
	return service.NewMemoryService(
		...
		nil, staleness.NewPolicyStore(db), nil, nil,
		nil, nil,
		store,
	)
}
...
func TestDuplicateGuard_ScopePreserved(t *testing.T) {
	...
	// Episodic (journal) is exempt: two identical journals both store. journal now
	// requires a date slug, so use two distinct dates.
	_, err = svc.StoreDocument(nctx, "journal", nil, "2026-09-01", content, false, "", nil, nil)
	require.NoError(t, err)
	res, err = svc.StoreDocument(nctx, "journal", nil, "2026-09-02", content, false, "", nil, nil)
	require.NoError(t, err)
