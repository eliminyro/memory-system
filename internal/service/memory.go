...
func (s *MemoryService) Search(ctx context.Context, query string, category, subcategory, docType *string, limit int, forceRead bool, reason string, overrideID *uuid.UUID, snippet bool) ([]repository.SearchResult, error) {
	...
	if penalty > 0 && s.thresholds != nil {
		if stalenessThresholds, err = s.thresholds.All(ctx); err != nil {
			return nil, err
		}
	}
...
func (s *MemoryService) StoreDocument(
	...
	// Duplicate guard: hash short-circuit (exact re-save) then a centroid-to-centroid
	// scan against same-tenant docs, excluding the target path so re-saves don't trip.
	// Only when enabled per-tenant, not forcing, and not episodic (journals exempt).
	...
	if settings.DuplicateGuard && !force && len(embeddings) > 0 && !models.IsEpisodic(docType) {
		...
		if hit != nil {
			...
			return &StoreResult{
				...
				Candidates: []repository.SimilarityCandidate{*hit},
			}, nil
		}
	...
	doc := &models.Document{
		...
		Title:          title,
		DocType:        docType,
		ContentHash:    contentHash,        // set on both create and update (doc reused below)
	...
	err = s.db.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		...
		if err == nil && existing.TenantID == tid {
			// Snapshot the prior doc+sections before the overwrite wipes them (F5).
			if recordHistory {
	...
	// Record create (nil Before) or overwrite (prior doc snapshot). Best-effort.
	if recordHistory {
	...
	var warnings []string
	if created && doc.DocType == models.DocTypeHandoff {
		if w := s.autoChainHandoff(ctx, tid, doc); w != "" {
...
// autoChainHandoff best-effort links a new handoff to the project's prior latest
// with a continues_from edge (new -> prior) in the write tenant. Never fails the
// store: errors are logged and returned as a warning. Create-only, idempotent.
func (s *MemoryService) autoChainHandoff(ctx context.Context, tid uuid.UUID, doc *models.Document) string {
	if s.edges == nil || s.docs == nil {
		return ""
	}
	prior, err := s.docs.LatestHandoff(ctx, []uuid.UUID{tid}, doc.Subcategory, false, &doc.ID)
	if err != nil {
		slog.Default().Warn("handoff auto-chain lookup failed", "document_id", doc.ID, "error", err)
		return fmt.Sprintf("auto-chain skipped: prior-handoff lookup failed: %v", err)
	}
	if prior == nil {
		return ""
	}
	...
	edge := &models.Edge{
		...
		EdgeType:         models.EdgeContinuesFrom,
		ActorSubject:     subj,
	}
	if _, _, err := s.edges.Create(ctx, edge); err != nil {
		slog.Default().Warn("handoff auto-chain link failed", "source", doc.ID, "target", prior.ID, "error", err)
