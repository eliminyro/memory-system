...
// TestCreateTenantDuplicateOwnerEmailIsInvalidInput: a personal tenant whose
// owner_email already belongs to another tenant is rejected as invalid input
// (400), not a raw 500 — the cross-tenant email-unique violation is classified.
func TestCreateTenantDuplicateOwnerEmailIsInvalidInput(t *testing.T) {
	db := openServicePG(t)
	store := authz.NewPostgresStore(db)
	svc := newAdminTestSvc(db, store)
	ctx := auth.WithLocalAdmin(context.Background())
	...
	_, err := svc.CreateTenant(ctx, "first-"+uuid.NewString(), email, models.TenantTypePersonal)
	require.NoError(t, err)

	_, err = svc.CreateTenant(ctx, "second-"+uuid.NewString(), email, models.TenantTypePersonal)
	require.ErrorIs(t, err, apperr.ErrInvalidInput, "duplicate owner_email must map to invalid input, not a 500")
}
