...
func TestListDocuments_Pagination(t *testing.T) {
	...
	all, err := f.svc.ListDocuments(ctx, &category, nil, nil, service.ListOptions{})
	require.NoError(t, err)
	require.Len(t, all, total, "limit<=0 must return every document")

	allNeg, err := f.svc.ListDocuments(ctx, &category, nil, nil, service.ListOptions{Limit: -1})
	require.NoError(t, err)
	require.Len(t, allNeg, total, "a negative limit is also unbounded")

	// A limit smaller than the set returns exactly one page.
	page0, err := f.svc.ListDocuments(ctx, &category, nil, nil, service.ListOptions{Limit: 3})
	require.NoError(t, err)
	require.Len(t, page0, 3, "limit=3 must return one page of 3")

	// Offset windows the result.
	tail, err := f.svc.ListDocuments(ctx, &category, nil, nil, service.ListOptions{Limit: 3, Offset: 6})
	require.NoError(t, err)
	require.Len(t, tail, 1, "offset=6 leaves a final short page of 1")

	beyond, err := f.svc.ListDocuments(ctx, &category, nil, nil, service.ListOptions{Limit: 3, Offset: total})
	require.NoError(t, err)
	...
	for off := 0; ; off += 2 {
		pg, err := f.svc.ListDocuments(ctx, &category, nil, nil, service.ListOptions{Limit: 2, Offset: off})
		require.NoError(t, err)
		for _, d := range pg {
