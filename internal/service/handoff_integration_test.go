...
func TestHandoff_SeedIdempotent(t *testing.T) {
	...
	var days int
	require.NoError(t, f.db.Raw(`SELECT days FROM staleness_thresholds WHERE doc_type = ?`, models.DocTypeHandoff).Scan(&days).Error)
	require.Equal(t, 3650, days, "handoff threshold seeded on migration")
	...
	require.NoError(t, f.db.Raw(`SELECT days FROM staleness_thresholds WHERE doc_type = ?`, models.DocTypeHandoff).Scan(&days).Error)
	require.Equal(t, 3650, days, "re-running the seed is idempotent")
}
...
func TestHandoff_AutoChainProjectScoped(t *testing.T) {
	...
	// A newer handoff under a real project must NOT be linked to by a later
	// project-less one (the bug a nil-means-any-project lookup would cause).
	storeHandoff(t, f, ctx, projA, "a-"+uuid.NewString(), "# A\n\n## summary\nA")

	g1, err := f.svc.StoreDocument(ctx, "handoffs", nil, "g1-"+uuid.NewString(), "# G1\n\n## summary\ng1", true, "seed", nil, nil)
	require.NoError(t, err)
	require.Empty(t, continuesFromTargets(t, f, ctx, g1.Document.ID),
		"a project-less handoff must not chain across a project")

	g2, err := f.svc.StoreDocument(ctx, "handoffs", nil, "g2-"+uuid.NewString(), "# G2\n\n## summary\ng2", true, "seed", nil, nil)
	require.NoError(t, err)
