...
// walk. The HTTP import worker wraps an unzipped-archive walk and drives the
// same ImportDocuments core through this one shape.
...
// seeding), so no separate seed step is needed here. Callers drive this loop
// via different DocSource implementations — e.g. the HTTP import worker's
// unzipped-archive walk.
