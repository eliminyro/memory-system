...
type tenantCreator interface {
	CreateTenant(ctx context.Context, name, ownerEmail string, tenantType ...string) (*models.Tenant, error)
}
...
// createTenantWithType backs POST /api/admin/tenants: decode name + optional
// owner_email (the personal owner's address) + optional type. An omitted type
// falls through to the service default (shared); an invalid type is 400.
func createTenantWithType(w http.ResponseWriter, r *http.Request, creator tenantCreator) {
	...
	if body.Type == models.TenantTypePersonal && body.OwnerEmail == "" {
		writeError(w, http.StatusBadRequest, "owner_email is required for a personal tenant")
		return
	}
	var (
	...
	if body.Type != "" {
		tenant, err = creator.CreateTenant(r.Context(), body.Name, body.OwnerEmail, body.Type)
	} else {
		tenant, err = creator.CreateTenant(r.Context(), body.Name, body.OwnerEmail)
	}
	if err != nil {
		writeErr(w, err)
		return
