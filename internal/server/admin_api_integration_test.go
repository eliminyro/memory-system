...
func TestAdminAPI_KeyLifecycleNeverLeaksSecret(t *testing.T) {
	...
	rec := httptest.NewRecorder()
	h.mux().ServeHTTP(rec, adminReq(t, adminCtx, http.MethodPost, "/admin/tenants", map[string]string{"name": "acme-" + uuid.NewString(), "type": models.TenantTypePersonal, "owner_email": "owner-" + uuid.NewString() + "@example.com"}))
