...
func (s *stubTenantSvc) CreateTenant(_ context.Context, name, email string, tenantType ...string) (*models.Tenant, error) {
	s.createName, s.createEmail, s.createType = name, email, tenantType
	if s.createErr != nil {
	...
	return &models.Tenant{ID: uuid.New(), Name: name, Email: email, Type: typ}, nil
}
...
func tenantUniverse() (entries []stubTenantEntry, acmeID, betaID, adaID uuid.UUID) {
	...
	entries = []stubTenantEntry{
		{access: service.TenantAccess{Tenant: models.Tenant{ID: acmeID, Name: "Acme Shared", Email: "acme@x.com", Type: models.TenantTypeShared}, Relation: "admin"}, managed: false},
		{access: service.TenantAccess{Tenant: models.Tenant{ID: betaID, Name: "Beta Shared", Email: "beta@x.com", Type: models.TenantTypeShared}, Relation: "manager"}, managed: true},
...
func TestCreateTenant_DefaultTypeOmitsArg(t *testing.T) {
	svc := &stubTenantSvc{}
	rec := doCreateTenant(svc, `{"name":"t","email":"e@x.com"}`)
