...
// the read view. expired sections show a preview and can only be
// verified, not edited.
function sectionEl(doc, s) {
  const container = el("div", { className: "sec" });
  const editable = s.status !== "expired";
  ...
  // expired — content withheld past its expiration age; only "Mark verified"
  ...
  if (!editable) {
    ...
    const previewText = (s.preview && s.preview !== heading)
      ? s.preview
      : "Content withheld — this section is past its expiration age and stays locked until verified against source.";
    withheld.append(el("p", { textContent: previewText }));
...
// ── Changes (mutation history) view ─────────────────────────────────────────
// Op → node/pill color + pill label. --op drives the timeline node, op pill and
// card-hover glow; colors resolve through the shared palette tokens.
const OP_META = {
  create: { color: "var(--ok)", label: "create" },
  update_section: { color: "var(--cool)", label: "edit section" },
  update_title: { color: "var(--violet)", label: "rename" },
  delete_section: { color: "var(--danger)", label: "delete section" },
  delete_document: { color: "var(--danger)", label: "delete document" },
};
// Filter-chip key → the op_types it shows (update_section + update_title = edited).
const FILTER_OPS = {
  created: ["create"],
  edited: ["update_section", "update_title"],
  deleted: ["delete_section", "delete_document"],
};
const CHG_FILTERS = [
  { key: "all", label: "all" },
  { key: "created", label: "created", color: "var(--ok)" },
  { key: "edited", label: "edited", color: "var(--cool)" },
  { key: "deleted", label: "deleted", color: "var(--danger)" },
];

// currentIdentity decodes the JWT access token the UI already holds to learn the
// signed-in subject/email — used only to mark a change "(you)". Returns null for
// an opaque/unparseable token; no extra endpoint is called.
let _identity;
function currentIdentity() {
  if (_identity !== undefined) return _identity;
  _identity = null;
  const parts = (sessionStorage.getItem("access_token") || "").split(".");
  if (parts.length === 3) {
    try {
      let s = parts[1].replace(/-/g, "+").replace(/_/g, "/");
      while (s.length % 4) s += "=";
      const p = JSON.parse(atob(s));
      _identity = { subject: p.sub || null, email: p.email || null };
    } catch (e) { _identity = null; }
  }
  return _identity;
}

// parseBefore returns the parsed `before` snapshot (shape varies by op_type), or
// null when absent/malformed. The wire value is a JSON string.
function parseBefore(entry) {
  if (!entry || !entry.before) return null;
  try { return JSON.parse(entry.before); } catch (e) { return null; }
}

// sectionText renders a heading+content pair as diffable text ("## H\ncontent").
function sectionText(heading, content) {
  const c = content || "";
  return heading ? "## " + heading + "\n" + c : c;
}

// buildDocText renders a whole document (title + sections) as diffable text.
function buildDocText(title, sections) {
  const parts = [];
  if (title) parts.push("# " + title);
  for (const s of sections || []) parts.push(sectionText(s.heading, s.content));
  return parts.join("\n");
}
...
// lineDiff is a hand-rolled LCS line diff → ordered {type:ctx|add|del,text} rows.
...
// diffEl builds a .diff block: a .hunk header + one .line per diff row.
function diffEl(hunk, lines) {
  const d = el("div", { className: "diff" }, el("div", { className: "hunk", textContent: hunk }));
  for (const ln of lines) {
    const g = ln.type === "add" ? "+" : ln.type === "del" ? "-" : " ";
    d.append(el("div", { className: "line " + ln.type },
      el("span", { className: "g", textContent: g }),
      el("span", { textContent: ln.text })));
  }
  return d;
}

// afterSection reconstructs a section's post-edit text: the `before` of the next
// NEWER update_section for the same section, else the live section, else "" (the
// section was deleted later). entries are newest-first.
function afterSection(entry, idx, entries, doc) {
  for (let j = idx - 1; j >= 0; j--) {
    const e = entries[j];
    if (e.op_type === "update_section" && e.section_id === entry.section_id) {
      const nb = parseBefore(e);
      return sectionText(nb && nb.heading, nb && nb.content);
    }
  }
  const live = (doc.sections || []).find((s) => s.id === entry.section_id);
  return live ? sectionText(live.heading, live.content) : "";
}

// afterTitle reconstructs the post-rename title: next newer update_title's
// `before`, else the live title.
function afterTitle(idx, entries, doc) {
  for (let j = idx - 1; j >= 0; j--) {
    if (entries[j].op_type === "update_title") {
      const nb = parseBefore(entries[j]);
      return (nb && nb.title) || "";
    }
  }
  return doc.title || "";
...
// diffFor builds the diff body for one entry, reconstructing before→after per
// op_type. Returns null when there is nothing truthful to show (a create with
// later edits shows no fabricated body).
function diffFor(entry, idx, entries, doc) {
  const b = parseBefore(entry);
  const h = b && b.heading;
  switch (entry.op_type) {
    case "update_section":
      return diffEl("@@ " + (h || "section") + " @@", lineDiff(sectionText(h, b && b.content), afterSection(entry, idx, entries, doc)));
    case "delete_section":
      return diffEl("@@ " + (h || "section") + " @@", lineDiff(sectionText(h, b && b.content), ""));
    case "update_title":
      return diffEl("@@ title @@", lineDiff((b && b.title) || "", afterTitle(idx, entries, doc)));
    case "delete_document":
      return diffEl("@@ deleted document @@", lineDiff(buildDocText(b && b.title, b && b.sections), ""));
    case "create": {
      if (entries.length !== 1) return null;
      const txt = buildDocText(doc.title, doc.sections);
      return txt ? diffEl("@@ new document @@", lineDiff("", txt)) : null;
    }
    default:
      return null;
  }
...
function headingBold(heading) {
  return el("b", { textContent: heading ? "## " + heading : "section" });
}

// targetEl builds the .target line naming what changed.
function targetEl(entry) {
  const b = parseBefore(entry);
  const t = el("div", { className: "target" });
  switch (entry.op_type) {
    case "update_section":
      t.append(document.createTextNode("edited "), headingBold(b && b.heading)); break;
    case "delete_section":
      t.append(document.createTextNode("removed section "), headingBold(b && b.heading)); break;
    case "update_title":
      t.append(document.createTextNode("changed the document title")); break;
    case "create":
      t.append(document.createTextNode("created the document")); break;
    case "delete_document": {
      const n = b && b.sections ? b.sections.length : 0;
      t.append(document.createTextNode("deleted the document" + (n ? " · " + n + " section" + (n === 1 ? "" : "s") : ""))); break;
    }
    default:
      return null;
  }
...
// whenText: compact relative age plus an absolute date, matching the doc meta.
function whenText(iso) {
  const rel = relAge(iso), abs = fmtDate(iso);
  if (rel && abs) return rel + " ago · " + abs;
  return abs || (rel ? rel + " ago" : "");
}

// historyCard builds one .chg: op-colored node + attributed card + diff body.
function historyCard(entry, idx, entries, doc) {
  const m = OP_META[entry.op_type] || { color: "var(--accent)", label: entry.op_type };
  const chg = el("div", { className: "chg" });
  chg.style.setProperty("--op", m.color);
  chg.append(el("span", { className: "node" }));

  const identity = entry.actor_email || entry.actor_subject || "";
  const local = (entry.actor_email || "").replace(/@.*/, "");
  const name = local || entry.actor_subject || "unknown";
  const avatar = el("span", { className: "avatar", textContent: (local || name).slice(0, 2).toUpperCase() || "?" });
  avatar.style.setProperty("--av", tenantColor(identity));

  const nm = el("span", { className: "nm", textContent: name });
  const me = currentIdentity();
  if (me && ((entry.actor_email && entry.actor_email === me.email) || (entry.actor_subject && entry.actor_subject === me.subject))) {
    nm.append(document.createTextNode(" "), el("span", { className: "you", textContent: "(you)" }));
  }
  const who = el("span", { className: "who" }, nm);
  if (entry.actor_email) who.append(el("small", { textContent: entry.actor_email }));

  const head = el("div", { className: "card-head" }, avatar, who,
    el("span", { className: "op", textContent: m.label }),
    el("span", { className: "when", textContent: whenText(entry.created_at) }));
  const card = el("div", { className: "card" }, head);
  const tgt = targetEl(entry);
  if (tgt) card.append(tgt);
  const diff = diffFor(entry, idx, entries, doc);
  if (diff) card.append(diff);
  chg.append(card);
  return chg;
}

// renderTimeline (re)draws the cards under the active op filter. idx stays the
// entry's index in the FULL newest-first array so reconstruction is correct.
function renderTimeline(timeline, doc, entries, filter) {
  timeline.replaceChildren();
  const ops = FILTER_OPS[filter];
  entries.forEach((entry, idx) => {
    if (ops && !ops.includes(entry.op_type)) return;
    timeline.append(historyCard(entry, idx, entries, doc));
  });
}

function buildFilterChips(entries, timeline, doc) {
  const wrap = el("div", { className: "filterchips" });
  const btns = [];
  for (const f of CHG_FILTERS) {
    const b = el("button", { className: "fchip" + (f.key === "all" ? " on" : ""), type: "button" });
    if (f.color) { const g = el("span", { className: "g" }); g.style.background = f.color; b.append(g); }
    b.append(document.createTextNode(f.label));
    b.addEventListener("click", () => {
      btns.forEach((x) => x.classList.remove("on"));
      b.classList.add("on");
      renderTimeline(timeline, doc, entries, f.key);
    });
    btns.push(b);
    wrap.append(b);
  }
...
// renderHistory fills the Changes panel: summary + filter chips + timeline, or a
// quiet empty state (history is toggle-gated, so [] is the normal case).
function renderHistory(panel, doc, entries) {
  panel.replaceChildren();
  if (!entries.length) {
    panel.append(el("p", { className: "state-msg", textContent: "No changes recorded for this document." }));
    return;
  }
  const contributors = new Set(entries.map((e) => e.actor_email || e.actor_subject)).size;
  const summary = el("span", { className: "chg-summary" },
    el("b", { className: "tnum", textContent: String(entries.length) }),
    document.createTextNode(" change" + (entries.length === 1 ? "" : "s") + " · "),
    el("b", { className: "tnum", textContent: String(contributors) }),
    document.createTextNode(" contributor" + (contributors === 1 ? "" : "s")));
  panel.append(el("div", { className: "chg-headrow" }, summary,
    el("span", { className: "who-scope", textContent: "newest first" })));
  const timeline = el("div", { className: "timeline" });
  panel.append(buildFilterChips(entries, timeline, doc), timeline);
  renderTimeline(timeline, doc, entries, "all");
}

// loadHistory lazily fetches the doc's mutation history on first Changes open and
// fills the tab count. [] is the common (toggle-off) case, not an error.
async function loadHistory(doc, panel, countEl) {
  panel.replaceChildren(el("p", { className: "state-msg", textContent: "loading…" }));
  let entries;
  try {
    entries = (await apiFetch("/documents/" + doc.id + "/history")) || [];
  } catch (err) {
    panel.replaceChildren(el("p", { className: "state-msg", textContent: "Couldn't load changes: " + err.message }));
    return;
  }
  if (countEl && entries.length) countEl.textContent = String(entries.length);
  renderHistory(panel, doc, entries);
}
...
async function showDocument(id) {
  ...
  const needing = sections.filter((s) => s.status === "needs_verification" || s.status === "expired").length;

  // Sub-tabs: Document (live sections, active by default) and Changes (mutation
  // history, lazy-loaded on first open). Both panels live here; one is shown.
  const docPanel = el("div", { className: "tab-panel" });
  ...
  docPanel.append(body);

  const chgCount = el("span", { className: "c tnum" });
  const docBtn = el("button", { className: "active", type: "button", textContent: "Document" });
  const chgBtn = el("button", { type: "button" }, document.createTextNode("Changes"), chgCount);
  let historyLoaded = false;
  const rocker = el("div", { className: "segmented" }, docBtn, chgBtn);
  view.append(rocker, docPanel, changesPanel);
  initRockers(view, (btn) => {
    const onChanges = btn === chgBtn;
    docPanel.hidden = onChanges;
    changesPanel.hidden = !onChanges;
    if (onChanges && !historyLoaded) { historyLoaded = true; loadHistory(doc, changesPanel, chgCount); }
  });
...
// ── Admin global-config tab (Task 8.3) ────────────────────────────────────────
// Admin-only page over GET/PATCH /api/admin/config: booleans → switch, 3–4-option
// enums → segmented, strings/numbers → .infield. No Save button (commit on change).

// CONFIG_SCHEMA mirrors the server's GlobalConfig grouped into panels. Client
// bounds (min/max/exclmin) match the shared validator; `depends` gates a row on a
// switch; `env`/`desc`/`def` are static labels ported from the approved design.
const CONFIG_SCHEMA = [
  ...
  { title: "Access & self-service", tc: "var(--ok)", fields: [
    { key: "self_service_policy", label: "Self-service policy", env: "MEMORY_SELF_SERVICE_POLICY", desc: "Who may edit tenant toggles & self-create API keys.", def: "default open", ctl: "seg", options: ["open", "admin_only"] },
    { key: "signup_domains", label: "Signup allowed domains", env: "SIGNUP_ALLOWED_DOMAINS", desc: "Comma-separated email domains that may self-provision a tenant.", def: "empty = public", ctl: "str", width: "wide", placeholder: "example.com, acme.org" },
    { key: "admin_emails", label: "Admin emails", env: "ADMIN_ALLOWED_EMAILS", desc: "Comma-separated emails granted admin at startup. Applied on restart; live grant/revoke lives in Tenants.", def: "restart-applied", ctl: "str", width: "wide", placeholder: "admin@example.com" },
  ] },
  ...
  { title: "HTTP hardening", tc: "var(--cool)", fields: [
    ...
    { key: "rate_limit_burst", label: "Burst", env: "RATE_LIMIT_BURST", def: "default 40 · min 1", ctl: "num", type: "int", min: 1 },
    { key: "trusted_proxy_depth", label: "Trusted proxy depth", env: "RATE_LIMIT_TRUSTED_PROXY_DEPTH", desc: "Trusted reverse-proxy hops in front.", def: "0 = key on RemoteAddr", ctl: "num", type: "int", min: 0, unit: "hops" },
    { key: "max_request_bytes", label: "Max request body", env: "MAX_REQUEST_BYTES", def: "0 disables · default 1048576", ctl: "num", type: "int", min: 0, width: "num-wide", unit: "bytes" },
  ] },
  { title: "Logging & notifications", tc: "var(--accent)", fields: [
    { key: "log_level", label: "Log level", env: "LOG_LEVEL", def: "default info", ctl: "seg", options: ["debug", "info", "warn", "error"] },
    { key: "webhook_url", label: "Webhook URL", env: "WEBHOOK_URL", desc: "Cleanup/sweep summaries are POSTed here as JSON.", def: "empty = disabled", ctl: "str", width: "wide", placeholder: "https://… (empty = disabled)" },
  ] },
];

// configValidate mirrors the server's bound checks; returns {ok,val} or {ok:false,msg}.
function configValidate(f, raw) {
  if (f.ctl === "str" || f.type === "string") return { ok: true, val: raw };
  raw = String(raw).trim();
  if (raw === "") return { ok: false, msg: "required" };
  const v = f.type === "int" ? (/^-?\d+$/.test(raw) ? parseInt(raw, 10) : NaN) : parseFloat(raw);
  if (Number.isNaN(v)) return { ok: false, msg: "not a number" };
  if (f.min !== undefined && (f.exclmin ? v <= f.min : v < f.min)) return { ok: false, msg: `must be ${f.exclmin ? ">" : "≥"} ${f.min}` };
  if (f.max !== undefined && v > f.max) return { ok: false, msg: `must be ≤ ${f.max}` };
  return { ok: true, val: v };
}

// renderConfig draws the admin global-config page from GET /admin/config; each
// control commits only its own field via PATCH, with a per-field flash + savebar.
async function renderConfig() {
  ...
  let cfg;
  try {
    cfg = await apiFetch("/admin/config");
  } catch (err) {
    if (seq === _renderSeq) showError(err);
    return;
  }
  ...
  const barTxt = el("span", { textContent: "all changes saved" });
  const savebar = el("span", { className: "savebar" }, el("span", { className: "led" }), barTxt);
  savebar.dataset.state = "idle";
  view.append(el("div", { className: "view-head" },
    el("h1", { className: "mem-title" }, document.createTextNode("Global config "),
      el("span", { className: "pill pill--cool", textContent: "admins only" })),
    savebar));

  let inflight = 0;
  const stamp = () => new Date().toLocaleTimeString([], { hour: "2-digit", minute: "2-digit", second: "2-digit" });
  const beginSave = () => { inflight++; savebar.dataset.state = "saving"; barTxt.textContent = "saving…"; };
  const endSave = (ok) => { if (--inflight <= 0) { inflight = 0; savebar.dataset.state = "idle"; barTxt.textContent = ok ? "saved " + stamp() : "save failed"; } };
  const patch = (key, value) => {
    beginSave();
    return apiFetch("/admin/config", { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ [key]: value }) })
      .then(() => { endSave(true); }).catch((err) => { endSave(false); throw err; });
  };
  const errMsg = (err) => (err && err.status === 403 ? "not allowed" : "save failed");
  const showFlash = (flash, ok, msg) => {
    flash.classList.toggle("err", !ok);
    flash.textContent = ok ? "saved" : msg;
    flash.classList.add("show");
    clearTimeout(flash._t);
    flash._t = setTimeout(() => flash.classList.remove("show", "err"), ok ? 1400 : 2600);
  };

  const gates = {}; // key → gate rocker el, for dependent gating
  const syncDeps = () => view.querySelectorAll("[data-depends]").forEach((row) => {
    const gate = gates[row.dataset.depends];
    const active = gate && gate.querySelector("button.active");
    const on = !!active && active.textContent.trim() === "on";
    row.classList.toggle("dim", !on);
    row.querySelectorAll("input").forEach((i) => { i.disabled = !on; });
  });
  const setSegActive = (seg, label) => {
    seg.querySelectorAll("button").forEach((b) => b.classList.toggle("active", b.textContent.trim() === label));
    placeThumb(seg, true);
  };

  const panels = el("div", { className: "config-panels" });
  for (const panel of CONFIG_SCHEMA) {
    const p = el("div", { className: "panel" });
    p.style.setProperty("--tc", panel.tc); // CSSOM (CSP forbids inline style attrs)
    const phead = el("div", { className: "panel-head" }, el("span", { className: "dot" }), el("h2", { textContent: panel.title }));
    if (panel.note) phead.append(el("span", { className: "eyebrow ph-note", textContent: panel.note }));
    p.append(phead);
    const sec = el("div", { className: "section" });
    for (const f of panel.fields) sec.append(configRow(f));
    p.append(sec);
    panels.append(p);
  }
  view.append(panels);

  // Segmented rockers commit on click; initRockers has already flipped .active, so
  // revert restores the last saved label from data-val on failure.
  initRockers(view, (btn, seg) => {
    const flash = seg.closest(".ctl").querySelector(".flash");
    const label = btn.textContent.trim();
    const val = seg.dataset.bool ? label === "on" : label;
    if (seg.dataset.bool) syncDeps();
    patch(seg.dataset.key, val)
      .then(() => { seg.dataset.val = label; showFlash(flash, true); })
      .catch((err) => { setSegActive(seg, seg.dataset.val); if (seg.dataset.bool) syncDeps(); showFlash(flash, false, errMsg(err)); });
  });
  placeThumbsIn(view, false);
  syncDeps();
  ...
  function configRow(f) {
    ...
    const k = el("span", { className: "k" }, document.createTextNode(f.label + " "));
    if (f.env) k.append(el("span", { className: "env", textContent: f.env }));
    const lbl = el("div", { className: "lbl" }, k);
    if (f.desc || f.def) {
      const small = el("small", { textContent: f.desc || "" });
      if (f.def) small.append(document.createTextNode(f.desc ? " · " : ""), el("b", { textContent: f.def }));
      lbl.append(small);
    }
    ...
    if (f.ctl === "seg") {
      ...
      const cur = f.bool ? (value ? "on" : "off") : (value == null ? "" : String(value));
      seg.dataset.val = cur;
      if (f.bool) { seg.dataset.bool = "1"; gates[f.key] = seg; }
      for (const opt of f.options) {
        const b = el("button", { type: "button", textContent: opt });
        if (opt === cur) b.classList.add("active");
        seg.append(b);
      }
      ctl.append(seg); // initRockers(view,…) wires the thumb + commit
    } else {
      const inp = el("input", {});
      if (f.type === "int") inp.inputMode = "numeric";
      else if (f.type === "float") inp.inputMode = "decimal";
      if (f.width === "wide") inp.className = "wide";
      else if (f.width === "num-wide") inp.className = "num-wide";
      if (f.placeholder) inp.placeholder = f.placeholder;
      inp.value = value == null ? "" : String(value);
      const box = el("div", { className: "infield" }, inp);
      if (f.unit) box.append(el("span", { className: "unit", textContent: f.unit }));
      ctl.append(box);
      let last = inp.value;
      const commit = () => {
        if (inp.value === last) return;
        const r = configValidate(f, inp.value);
        if (!r.ok) { box.classList.add("err"); showFlash(flash, false, r.msg); return; }
        box.classList.remove("err");
        patch(f.key, r.val).then(() => {
          last = inp.value;
...
function keysSection(t, keys, refresh) {
  ...
  const ttlI = el("input", { type: "number", min: "1", inputMode: "numeric" });
  const ttlField = el("div", { className: "infield" }, ttlI, el("span", { className: "unit", textContent: "days" }));
  ...
  const cfg = el("div", { className: "key-config" },
    ...
    el("div", { className: "af-row" }, el("label", { textContent: "TTL" }), ttlField, el("span", { className: "hint", textContent: "optional — blank never expires" })),
