...
func TestHybridSearch_MMRDisplacesNearDuplicate(t *testing.T) {
	...
	// MMR on (moderate lambda): diversity displaces B for the lower-scoring
	// but diverse C, while A (top relevance) is still picked first. Only
	// possible if s.embedding threaded correctly through both CTEs to Go.
	lambda := 0.5
	mmrParams := base
