...
func (r *LintRepository) CheckStale(ctx context.Context, tenantID uuid.UUID, thresholds LintThresholds, excludeDocTypes []string) ([]LintFinding, error) {
	...
	var rows []row
	excluded := nonNilArray(excludeDocTypes)
	if err := r.db.WithContext(ctx).Raw(sql, tenants, excluded, thresholds.StaleDays).Scan(&rows).Error; err != nil {
...
func (r *LintRepository) FindNearDuplicatePairs(ctx context.Context, tenantID uuid.UUID, threshold float64, excludeDocTypes []string) ([]NearDuplicatePair, error) {
	...
	if err := r.runBoundedScan(ctx, &pairs, sql,
		tenantID, excluded, maxSections, tenantID, excluded, neighbors, threshold, maxPairs); err != nil {
...
func (r *LintRepository) CheckNearDuplicates(ctx context.Context, tenantID uuid.UUID, thresholds LintThresholds, excludeDocTypes []string) ([]LintFinding, error) {
	tenants := readTenants(tenantID)
	...
	if err := r.runBoundedScan(ctx, &rows, sql,
		tenants, excluded, maxSections, tenants, excluded, neighbors, thresholds.DuplicateSimilarity, maxPairs); err != nil {
