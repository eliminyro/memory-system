...
func (r *SectionRepository) HybridSearch(ctx context.Context, p SearchParams) ([]SearchResult, error) {
	...
	args := []any{
		vec, tenants, p.Category, p.Category, p.Subcategory, p.Subcategory, p.DocType, p.DocType, vec, pool, // semantic
		p.Query, tenants, p.Category, p.Category, p.Subcategory, p.Subcategory, p.DocType, p.DocType, p.Query, pool, // keyword
	}
