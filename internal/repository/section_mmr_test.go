...
// TestApplyMMR_NearDuplicateDisplacedAtLimit is the spec's canonical scenario:
// two near-identical high scorers A, B (cosine(A,B) ~ 0.99997) plus a
// lower-scoring but diverse C (cosine(A,C) ~ 0.649). At limit=2, moderate
// lambda, MMR keeps one of {A,B} plus C rather than both A and B.
func TestApplyMMR_NearDuplicateDisplacedAtLimit(t *testing.T) {
	a := SearchResult{SectionID: uuid.New(), Score: 0.99}
	b := SearchResult{SectionID: uuid.New(), Score: 0.989}
	c := SearchResult{SectionID: uuid.New(), Score: 0.75}
	scored := []SearchResult{a, b, c} // already score-sorted desc, as fuseHybridScored guarantees
	...
	got := applyMMR(scored, embs, 0.5, 2)
...
func TestApplyMMR_MembershipPreservedCountMinNL(t *testing.T) {
	...
	for _, limit := range []int{0, 3, 5, 10} {
		got := applyMMR(scored, embs, 0.7, limit)
		want := len(scored)
