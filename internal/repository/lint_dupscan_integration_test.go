...
func TestFindNearDuplicatePairs_Integration(t *testing.T) {
	...
	start := time.Now()
	pairs, err := lr.FindNearDuplicatePairs(context.Background(), tenantID, models.ScanThreshold, nil)
...
func TestCheckNearDuplicates_PairCap_Integration(t *testing.T) {
	...
	all, err := lr.CheckNearDuplicates(context.Background(), tenantID, th, nil)
	if err != nil {
	...
	capped, err := lr.CheckNearDuplicates(context.Background(), tenantID, th, nil)
	if err != nil {
