...
a populated corpus (even at the same dimension), because cosine similarity and the
duplicate guard are only comparable within one embedding space.
...
4. **Re-embed.** Re-import your source documents (via the admin import — web UI
   or `POST /import`); each section is embedded with the new model into the fresh
   corpus.
