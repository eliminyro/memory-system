...
func armBootstrapToken(hasAdmin bool, logger *slog.Logger) (string, error) {
	...
	logger.Warn("instance not bootstrapped: POST this one-time token to /bootstrap to provision the first admin; regenerated every boot, never persisted",
		"bootstrap_token", token)
	return token, nil
}

// globalConfigSeed maps env-derived config into the instance_config bootstrap
// seed applied at migrate time (a stored value then wins).
func globalConfigSeed(cfg *config.Config) database.GlobalConfigDefaults {
	return database.GlobalConfigDefaults{
		...
		SelfServicePolicy:     cfg.SelfServicePolicy,
		SignupDomains:         cfg.SignupAllowedDomainsSpec,
		...
		WebhookURL:            "",
		RequireConfigListener: cfg.RequireConfigListener,
	}
}

// applyLogLevel parses a validated slog level name and applies it to lv, so a
// log_level PATCH retunes the shared handler live (a bad value is validated out
// upstream; UnmarshalText leaves lv unchanged on error).
...
func main() {
	...
	// Shared LevelVar so a log_level config PATCH can retune the running handler.
	...
	// Global runtime config + doc-type policy store: atomic snapshots loaded at
	...
	// startup is never missed. A start failure is logged, not fatal.
	sqlDB, err := db.DB()
	if err != nil {
		slog.Error("failed to get sql.DB for config listener", "error", err)
		os.Exit(1)
	}
	configListener := pgnotify.New(sqlDB, slog.Default())
	configListener.Register(models.InstanceConfigNotifyChannel, globalCfg.Load)
	configListener.Register(models.DocTypePoliciesNotifyChannel, policyStore.Recompute)
	if err := configListener.Start(rootCtx); err != nil {
		slog.Warn("config invalidation listener failed to start; replica may serve stale config until it reconnects", "error", err)
	}

	if err := globalCfg.Load(context.Background()); err != nil {
		slog.Error("failed to load global config", "error", err)
		os.Exit(1)
	}
	// A stored log_level wins over the env bootstrap value used above.
	applyLogLevel(logLevel, globalCfg.LogLevel())

	// A bad policy row is fatal — fail fast, naming the doc_type and field.
	if err := policyStore.Load(context.Background()); err != nil {
		slog.Error("failed to load doc_type policies", "error", err)
		os.Exit(1)
	}
	...
	// startup; never consulted per request (all decisions go through tuple Check).
	var allowedEmails []string
	if spec := globalCfg.AdminEmails(); spec != "" {
		allowedEmails = strings.Split(spec, ",")
	}
	...
	// Cleanup pipeline — near-duplicate scan + webhook summary. The scanner reads
	...
	// deadKeyTTLDays so retired keys stop cluttering listings. Shares the cleanup
	// cadence and rootCtx lifecycle; sweeps once at startup then on the interval.
	const deadKeyTTLDays = 7
	deadKeyTTL := time.Duration(deadKeyTTLDays) * 24 * time.Hour
	...
	if cfg.AuthletEnabled() {
		...
		authletWiring.Resolver.Provision = func(ctx context.Context, c idp.Claims) (string, error) {
			...
			tid, provErr := memorySvc.ProvisionPersonalTenant(ctx, c.Email, c.Name, hd, config.ParseAllowedDomains(globalCfg.SignupDomains()), c.Issuer, c.Subject)
	...
	handler := server.NewHandler(server.Deps{
		...
		InstanceConfig: instanceConfigRepo,
