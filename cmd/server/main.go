...
func main() {
	...
	// Global runtime config: an atomic snapshot of instance_config, loaded at
	// boot and refreshed on write. Shared by the service + server layers.
	...
	cleanupScanner := cleanup.NewScanner(
		lintRepo, tenantRepo, cleanupRepo, mutationHistoryRepo,
